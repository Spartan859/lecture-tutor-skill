...
- **具体数值例子** — 每个重要概念配具体计算实例，不只是抽象定义
- **讲义定位回查（最大特色）** — 每个知识点精确标注 `[文件名] 第X页 [章节/定理号]`，一键回查原课件对应位置，建立讲解文档与源课件之间的精确映射。讲解作业时还能关联到课件中对应的章节，告诉你"这道题考的是课件第几页的内容"
...
### 2. 直观理解
用通俗语言解释含义、直观直觉、为什么需要这个概念、常见误区，并配具体计算实例。
...
- **数值例子**：每个重要概念配具体计算
- **直观理解**：至少一段话（5-8 句），涵盖直观含义、类比解释、常见误区
